...
export const Falling = () => {
  ...
  useEffect(() => {
    ...
    Composite.add(engine.world, [wallR, wallL, wallB, ...customItems, mouseConstraint])
      ...
      <WordStyle ref={boxRef}>
        ...
        {infos.map((info, index) =>
          info.category === 'about-kr'
            ? info.items.map((item, _index) => (
                <span ref={(el) => (aboutKrRef.current[_index] = el)} className={`word word--about word--about-kr`} key={`${index}-${_index}`}>
                  {item}
                </span>
              ))
            : info.category === 'about-en'
            ? info.items.map((item, _index) => (
                <span ref={(el) => (aboutEnRef.current[_index] = el)} className={`word word--about word--about-en`} key={`${index}-${_index}`}>
                  {item}
                </span>
              ))
            : info.category === 'menu'
            ...
            : info.items.map((item, _index) => (
                <span ref={(el) => (aboutRef.current[_index] = el)} className={`word word--about word--${info.category}`} key={`${index}-${_index}`}>
                  {item}
                </span>
              ))
        )}
